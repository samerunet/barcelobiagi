...
export function Category() {
  ...
  const baseProducts = useMemo(() => {
    ...
    if (category === 'premium') return mockProducts.filter(p => p.tags.includes('premium'));
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedColors.length > 0) {
      filtered = filtered.filter(p =>
        selectedColors.includes(language === 'ru' ? p.color_ru : p.color_en)
      );
    }
  ...
  const availableColors = Array.from(
    new Set(baseProducts.map(p => (language === 'ru' ? p.color_ru : p.color_en)))
  );
